...
def clear_log_dir(air):
    """"
        清理log文件夹 xxx.air/log
        Remove folder xxx.air/log
    """
...
def get_log_dir(device, air):
    """"
        在 xxx.air/log/ 文件夹下创建每台设备的运行日志文件夹
...
if __name__ == '__main__':
    """
        ...
        Init variables here
    """
    airList = ['company\company_search.air', 'company\company_baseInfo.air', 'company\company_risk.air',
               'company\company_brand.air', 'company\company_graph.air', 'company\company_context.air',
               'company\company_riskInfo.air',
               'company\company_develop.air', 'company\company_manage.air', 'company\company_property.air',
               'company\company_history.air',
               'company\company_discuss.air', 'human\human_search.air', 'human\human_risk.air',
               'human\human_controller.air']

    devices = [tmp[0] for tmp in ADB().devices()]
    for num in range(len(airList)):
        air = airList[num]
